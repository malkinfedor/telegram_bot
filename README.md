...
**Для деплоя данного бота через ansible следует сделать следующее** (тестировалось только на centos 7):**
...
- *ansible-playbook -i inventory.yml  deploy_app.yml* - запустить ansible's playbook который установит необходимые утилиты ОС, приложение, запустит приложение и добавит его в автозагрузку
Примечание: это нулевая версия данного плейбука, ничего не вынесено в vars и даже пользователь задан "железно". В следующей версии сделаю роль, в которой все значения будут задаваться через переменные.
